...
void ENTER(char *pFunc)
{
	...
	sprintf( &aText[ TRACE_Indent ], "%s\n", pFunc );
	OutputDebugString( aText );

	TRACE_Indent++;
}
void LEAVE(char *pFunc)
{
	TRACE_Indent--;
}
#else
inline void ENTER(char *pFunc)
{
}
inline void LEAVE(char *pFunc)
{
}
#endif
...
// Misc. __________________________________________________________________________________________
	...
	// NULL = normal debugger screen,
	// else Update*Cell()
	VideoUpdateFuncPtr_t g_pfDebuggerRenderUpdate = NULL;
...
//	_Video_RedrawScreen( pfUpdate ); /// vvv
	g_pfDebuggerRenderUpdate = pfUpdate;
...
void DebugBegin ()
{
	...
	g_pfDebuggerRenderUpdate = NULL;

	// DD Full-Screen Palette
	// Commenting this out, even the debug cursor blinking fixes the palette
...
//void DebugProcessCommand (int keycode)
{
	...
	{
		// pressing shift, control, or alt by it self shouldn't end viewig Apple output
...
//					}
	...
	bUpdateDisplay |= UPDATE_CONSOLE_DISPLAY | UPDATE_CONSOLE_INPUT; // vvv
...
//		void UpdateDisplay (Update_t bUpdate);
//Update_t bUpdateDisplay = UPDATE_CONSOLE_INPUT; // vvv
//if (bUpdateDisplay && !g_bDebuggerViewingAppleOutput) // vvv
//{
//	UpdateDisplay( bUpdateDisplay ); // vvv
//}
...
//	DebugDisplay( UPDATE_CONSOLE_DISPLAY ); // vvv
}
