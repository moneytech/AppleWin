...
/* Description: Emulation of Apple video modes
 *
 * Author: Various
 */
...
// Types ____________________________________________________________
	...
	// Pre-set "Monochromes"
		...
		, LOGO_COLORS_START
		, LOGO_COLORS_END = LOGO_COLORS_START + 127

		, NUM_COLOR_PALETTE
	};


	enum RenderAppleModes_e
	{
		R_TEXT40, // Text mode
		R_TEXT80, // Text mode
		R_SGR,  // (Single) Graphics Mode
		R_DGR,  // (Double) Graphics Mode
		R_SHGR, // (Single) Hi-Res
		R_DHGR, // (Double) Hi-Res
	};
...
// Globals __________________________________________________________
	...
	// CellSize.X = 7
	// 280 pixels = CellSize.X * NumCells.X
	const int CELLS_X = 40;
	const int CELLS_Y = 24;
	const int TOTAL_CELLS = CELLS_X * CELLS_Y;

	static BYTE          celldirty[CELLS_Y][CELLS_X];
	...
	static int			g_nAltCharSetOffset  = 0; // alternate character set
			...
			bool		g_bVideoForceFullRedraw = 1;

			LPBYTE		g_pFrameBuffer  = (LPBYTE)0;
			LONG		g_nFramePitch = 0;
                           ...
                           int sourcex, int sourcey)
{
  ...
  while (ysize--)
  {
    ...
    currdestptr   -= g_nFramePitch;
    currsourceptr -= SRCOFFS_TOTAL;
  }
}

//===========================================================================
void Video_CreateFrameOffsetTable (const LPBYTE nFrameBufferAddress, const LONG nFramePitch)
{
//	if ((g_pFrameBuffer  == nFrameBufferAddress) &&
//		(g_nFramePitch == nFramePitch))
//	{
//		return;
//	}
	...
	{
		frameoffsettable[y] = g_pFrameBuffer + g_nFramePitch*((FRAMEBUFFER_H-1)-y);
	}
}
...
//	pTmp += (DEBUG_COLORS_START * 4);
	pTmp = (BYTE*) &g_pFramebufferinfo->bmiColors[ DEBUG_COLORS_START ];
...
bool Update40ColCell (int x, int y, int xpixel, int ypixel, int offset)
{
	...
	bool bCharChanged = (ch != *(vidlastmem+offset+0x400) || g_bVideoForceFullRedraw);
...
bool Update80ColCell (int x, int y, int xpixel, int ypixel, int offset)
{
	...
	bool bC1Changed = (c1 != *(vidlastmem + offset +     0) || g_bVideoForceFullRedraw);
	bool bC0Changed = (c0 != *(vidlastmem + offset + 0x400) || g_bVideoForceFullRedraw);
...
void __stdcall CopyMixedSource (int x, int y, int sourcex, int sourcey) {	// For tv emulation g_nAppMode
  ...
  for (count = 0, bufxoffset = 0; 
	...
	if (hgrlinesabove)
		currptr += g_nFramePitch << 1;

    for (i = istart;
	     i <= iend; 
	     currptr -= g_nFramePitch, i++) {
...
bool UpdateLoResCell (int x, int y, int xpixel, int ypixel, int offset)
{
	...
	if ((val != *(vidlastmem+offset+0x400)) || g_bVideoForceFullRedraw)
	{
		CopySource(xpixel,ypixel,
			14,8,
...
{
	if (SW_MIXED || g_bVideoForceFullRedraw)
		return 1;
...
void VideoBenchmark () {
  ...
  do {
    ...
    VideoRedrawScreen(); // was  VideoRefreshScreen
    if (cycle++ >= 3)
      cycle = 0;
    totaltextfps++;
  ...
  do {
    ...
    if (cycle & 1)
      ...
      CopyMemory(mem+0x2000,mem+((cycle & 2) ? 0x4000 : 0x6000),0x2000);
    VideoRedrawScreen(); // VideoRedrawScreen VideoRefreshScreen
    if (cycle++ >= 3)
      cycle = 0;
    realisticfps++;
  } while (GetTickCount() - milliseconds < 1000);
...
//	HDC g_hFrameDC = FrameGetDC(); // vvv
	...
	{
		SelectObject(g_hFrameDC,brush);
		SelectObject(g_hFrameDC,GetStockObject(NULL_PEN));
		Rectangle(g_hFrameDC,0,0,FRAMEBUFFER_W+1,FRAMEBUFFER_H+1);
	}
...
#define  DRAWVERSION(x,y,c)     \
	SetTextColor(g_hFrameDC,c);   \
	TextOut(g_hFrameDC,           \
		540+x,358+y,            \
		szVersion,              \
		strlen(szVersion));

	if (GetDeviceCaps(g_hFrameDC,PLANES) * GetDeviceCaps(g_hFrameDC,BITSPIXEL) <= 4) {
	...
	if( g_bIsFullScreen )	
	{
		if( !g_pDDPal )
		{
			...
			for(iPal = 0; iPal < 256; iPal++ )
			{
				...
				*(pDst + 1) = *(pSrc + 1);
				*(pDst + 2) = *(pSrc + 0);
				*(pDst + 3) = 0; // PC_NOCOLLAPSE;
				pDst += 4;
				pSrc += 4;
			}
			DWORD nDDP = DDPCAPS_8BIT | DDPCAPS_ALLOW256 | DDPCAPS_INITIALIZE | DDPCAPS_PRIMARYSURFACE;
		...
		if (g_pDDPal)
		{
			...
			// To modify the palette after
			// g_pDDPal->SetPaletteEntries(0,iFirst,nColors,pNewColorsPalEntries)
		}
	}
	else
#endif
...
void _Video_Dirty()
{
	ZeroMemory(celldirty, TOTAL_CELLS);
}
...
void VideoRefreshScreen ()
{
	...
	{
		_Video_RedrawScreen( g_pfDebuggerRenderUpdate, false );
		return;
	}
...
{
//  HDC    g_hFrameDC = FrameGetVideoDC(&g_pFrameBuffer,&g_nFramePitch);
//  CreateFrameOffsetTable(g_pFrameBuffer,g_nFramePitch); // ptr to start of each scanline
  ...
  int  ypixel   = 0;
	...
	// 320 Scan lines: 20 lines * 16 pixels/line
	// +64 Scan lines:  4 lines * 16 pixels/line
	//====
	// 384 Scanlines (doubled from 192 to maintain aspect ratio)
	...
	while (y < 20)
	{
		...
		int xpixel = 0;
		while (x < CELLS_X) {
		  anydirty |= celldirty[y][x] = pfUpdate(x,y,xpixel,ypixel,offset+x);
		  ++x;
		  xpixel += 14;
		}
		++y;
	...
	if( g_uHalfScanLines )
	{
		...
		{	
			...
			// Pointer 4 byte aligned?
			if (((unsigned int)pSrc & 3) == 0)
			{
				unsigned int *pDst = (unsigned int *) pSrc;
				for( int x = 0; x < FRAMEBUFFER_W / 4; x++ )
				{
					*pDst++ = 0;
				}
			}
	...
	if( g_uScanLines == 75 )
	{
		// 0..191 = 192 scan lines
		// (0,382)

		// We zero out the 1st row (row zero) because the bitmap is upside down
		for( int y = 0; y < FRAMEBUFFER_H; y += 2 )
		{	
			unsigned char *pSrc = frameoffsettable[y+0];
			unsigned char *pDst = frameoffsettable[y+1];
			for( int x = 0; x < FRAMEBUFFER_W; x++ )
			{
				// TODO: Move Dark color palette to 0x80 so can use |= 0x80
				switch( *pSrc )
				{
					case HGR_WHITE:      
						*pDst = LIGHT_GRAY;
	...
	if (g_hFrameDC && anydirty)
	{
		BitBlt(g_hFrameDC,0,0,FRAMEBUFFER_W,FRAMEBUFFER_H,g_hDeviceDC,0,0,SRCCOPY); 
		GdiFlush();
	}
#else

	// Original code:
	if (g_bIsFullScreen || !anydirty) // !g_hFrameDC
	{
		SetLastDrawnImage();
//		g_bVideoForceFullRedraw = 0;
		return;
	}
	...
	while (x < 40) {
	  if ((x == 39) && celldirty[y][x])
		if (start >= 0) {
		  ...
		  remainingdirty = 1;
	  if ((start >= 0) && !celldirty[y][x]) {
		if ((x - startx > 1) || ((x == 39) && (xpixel == FRAMEBUFFER_W))) {
		  int height = 1;
		  while ((y+height < 24)
				   && celldirty[y+height][startx           ]
				   && celldirty[y+height][x-1              ]
				   && celldirty[y+height][(startx+x-1) >> 1] )
			height++;
		  BitBlt(
				g_hFrameDC, // g_hFrameDC
				start,ypixel,xpixel-start,height << 4,
				g_hDeviceDC,start,ypixel,SRCCOPY);
		  while (height--) {
			int loop = startx;
			while (loop < x+(xpixel == FRAMEBUFFER_W))
			  celldirty[y+height][loop++] = 0;
		  }
		  start = -1;
		}
		else
	  ...
	  else if ((start == -1) && celldirty[y][x] && (x < 39)) {
		start  = xpixel;
		startx = x;
	  }
	  x++;
  ...
  if (remainingdirty) {
	...
	while (x < 40) {
	  ...
	  while (y < 24) {
		...
		if ((start >= 0) && !celldirty[y][x]) {
		  BitBlt(
				g_hFrameDC,xpixel,start,14,ypixel-start, // g_hFrameDC
				g_hDeviceDC,xpixel,start,SRCCOPY);
		  start = -1;
		}
		else if ((start == -1) && celldirty[y][x])
		  start = ypixel;
		y++;
		ypixel += 16;
	  }
...
BYTE __stdcall VideoSetMode (WORD, WORD address, BYTE write, BYTE, ULONG nCyclesLeft)
{
  ...
  int   oldvalue = g_nAltCharSetOffset+(int)(gnCurrentVideoMode & ~(VF_MASK2 | VF_PAGE2));
  switch (address) {
    case 0x00: gnCurrentVideoMode &= ~VF_MASK2;   break;
    case 0x01: gnCurrentVideoMode |=  VF_MASK2;   break;
    case 0x0C: if (!IS_APPLE2) gnCurrentVideoMode &= ~VF_80COL;   break;
    case 0x0D: if (!IS_APPLE2) gnCurrentVideoMode |=  VF_80COL;   break;
    case 0x0E: if (!IS_APPLE2) g_nAltCharSetOffset = 0;           break;	// Alternate char set off
    case 0x0F: if (!IS_APPLE2) g_nAltCharSetOffset = 256;         break;	// Alternate char set on
    case 0x50: gnCurrentVideoMode &= ~VF_TEXT;    break;
    case 0x51: gnCurrentVideoMode |=  VF_TEXT;    break;
    case 0x52: gnCurrentVideoMode &= ~VF_MIXED;   break;
    case 0x53: gnCurrentVideoMode |=  VF_MIXED;   break;
    case 0x54: gnCurrentVideoMode &= ~VF_PAGE2;   break;
    case 0x55: gnCurrentVideoMode |=  VF_PAGE2;   break;
    case 0x56: gnCurrentVideoMode &= ~VF_HIRES;   break;
    case 0x57: gnCurrentVideoMode |=  VF_HIRES;   break;
    case 0x5E: if (!IS_APPLE2) gnCurrentVideoMode |=  VF_DHIRES;  break;
    case 0x5F: if (!IS_APPLE2) gnCurrentVideoMode &= ~VF_DHIRES;  break;
  }
  if (SW_MASK2)
    gnCurrentVideoMode &= ~VF_PAGE2;

  if (oldvalue != g_nAltCharSetOffset+(int)(gnCurrentVideoMode & ~(VF_MASK2 | VF_PAGE2))) {
...
/*
  ...
  if (oldpage2 != SW_PAGE2) {
    ...
    if ((g_bVideoDisplayPage2 && !SW_PAGE2) || (!behind)) {
      g_bVideoDisplayPage2 = (SW_PAGE2 != 0);
      if (!g_bVideoForceFullRedraw) {
        VideoRefreshScreen();
        hasrefreshed = 1;
        lastrefresh  = emulmsec;
      }
    }
    else if ((!SW_PAGE2) && (!g_bVideoForceFullRedraw) && (emulmsec-lastrefresh >= 20)) {
      g_bVideoDisplayPage2 = 0;
      VideoRefreshScreen();
      hasrefreshed = 1;
      lastrefresh  = emulmsec;
...
DWORD VideoGetSnapshot(SS_IO_Video* pSS)
{
	...
	pSS->dwVidMode = gnCurrentVideoMode;
	return 0;
}
...
DWORD VideoSetSnapshot(SS_IO_Video* pSS)
{
	...
	gnCurrentVideoMode = pSS->dwVidMode;
