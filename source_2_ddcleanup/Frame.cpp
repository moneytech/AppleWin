...
void    DrawStatusArea (HDC hDC, BOOL drawflags);
...
//===========================================================================
static void Draw3dRect (HDC hDC, int x1, int y1, int x2, int y2, BOOL out)
{	
	SelectObject(g_hFrameDC,GetStockObject(NULL_BRUSH));
	SelectObject(g_hFrameDC,out ? btnshadowpen : btnhighlightpen);
	POINT pt[3];
	pt[0].x = x1;    pt[0].y = y2-1;
	pt[1].x = x2-1;  pt[1].y = y2-1;
	pt[2].x = x2-1;  pt[2].y = y1; 
	Polyline(g_hFrameDC,(LPPOINT)&pt,3);
	SelectObject(g_hFrameDC,(out == 1) ? btnhighlightpen : btnshadowpen);
	pt[1].x = x1;    pt[1].y = y1;
	pt[2].x = x2;    pt[2].y = y1;
	Polyline(g_hFrameDC,(LPPOINT)&pt,3);
}
...
//===========================================================================
static void DrawBitmapRect (HDC hDC, int x, int y, LPRECT rect, HBITMAP bitmap) {
  HDC memg_hFrameDC = CreateCompatibleDC(g_hFrameDC);
  SelectObject(memg_hFrameDC,bitmap);
  BitBlt(g_hFrameDC,x,y,
         ...
         rect->top,
         SRCCOPY);
  DeleteDC(memg_hFrameDC);
}

//===========================================================================
static void DrawButton (HDC hDC, int number)
{
//	FrameReleaseDC();
//	HDC g_hFrameDC = (hDC ? hDC : GetDC(g_hFrameWindow));
	int x  = buttonx;
	...
	if (number == buttondown)
	{
		...
		while (loop++ < 3)
		{
			Draw3dRect(g_hFrameDC,x+loop,y+loop,x+BUTTONCX,y+BUTTONCY,0);
		}
		RECT rect = {0,0,39,39};
		DrawBitmapRect(g_hFrameDC,x+4,y+4,&rect,buttonbitmap[number]);
	}
	else
	{
		Draw3dRect(g_hFrameDC,x+1,y+1,x+BUTTONCX,y+BUTTONCY,1);
		Draw3dRect(g_hFrameDC,x+2,y+2,x+BUTTONCX-1,y+BUTTONCY-1,1);
		RECT rect = {1,1,40,40};
		DrawBitmapRect(g_hFrameDC,x+3,y+3,&rect,buttonbitmap[number]);
	}
	if ((number == BTN_DRIVE1) || (number == BTN_DRIVE2))
	{
		...
		RECT rect = {x+offset+3,
				 ...
				 x+offset+42,
				 y+offset+42};
		SelectObject(g_hFrameDC,smallfont);
		SetTextColor(g_hFrameDC,RGB(0,0,0));
		SetTextAlign(g_hFrameDC,TA_CENTER | TA_TOP);
		SetBkMode(g_hFrameDC,TRANSPARENT);
		ExtTextOut(
			g_hFrameDC,x+offset+22,rect.top,ETO_CLIPPED,&rect,
...
static void DrawCrosshairs (int x, int y)
{
  ...
  static int lasty = 0;
//FrameReleaseDC();
//  HDC g_hFrameDC = GetDC(g_hFrameWindow);
//	HDC g_hFrameDC = g_hFrameDC;


#define LINE(x1,y1,x2,y2) MoveToEx(g_hFrameDC,x1,y1,NULL); LineTo(g_hFrameDC,x2,y2);
  ...
  if (lastx && lasty)
    if (g_bIsFullScreen) {
      ...
      while (loop--) {
        ...
        FillRect(g_hFrameDC,&rect,(HBRUSH)GetStockObject(BLACK_BRUSH));
      }
    }
    else {
      int loop = 5;
      while (loop--) {
        switch (loop) {
          case 0: SelectObject(g_hFrameDC,GetStockObject(BLACK_PEN));  break;
          case 1: // fall through
          case 2: SelectObject(g_hFrameDC,btnshadowpen);               break;
          case 3: // fall through
          case 4: SelectObject(g_hFrameDC,btnfacepen);                 break;
        }
        ...
        if ((loop == 1) || (loop == 2))
          SelectObject(g_hFrameDC,btnhighlightpen);
  ...
  if (x && y) {
    ...
    while (loop--) {
      if ((loop == 1) || (loop == 2))
        SelectObject(g_hFrameDC,GetStockObject(WHITE_PEN));
      else
        SelectObject(g_hFrameDC,GetStockObject(BLACK_PEN));
...
//	PAINTSTRUCT ps;
//	HDC         g_hFrameDC = (g_bPaintingWindow
...
//	HDC g_hFrameDC = g_hFrameDC;
	...
	VideoRealizePalette(g_hFrameDC);

	if (!g_bIsFullScreen)
	{
		// DRAW THE 3D BORDER AROUND THE EMULATED SCREEN
		Draw3dRect(g_hFrameDC,
		...
		Draw3dRect(g_hFrameDC,
			...
			0);
		SelectObject(g_hFrameDC,btnfacepen);
		Rectangle(g_hFrameDC,
		...
		while (iButton--)
		{
			DrawButton(g_hFrameDC,iButton);
		}
	}

	// DRAW THE STATUS AREA
	DrawStatusArea(g_hFrameDC,DRAW_BACKGROUND | DRAW_LEDS);
...
//if (!g_bIsFullScreen)
	...
	if (g_nAppMode == MODE_LOGO)
		VideoDisplayLogo();
	else if ((g_nAppMode == MODE_DEBUG) && !g_bDebuggerViewingAppleOutput) // vvv
		DebugDisplay(1);
	else
	{
		VideoRedrawScreen();
...
//===========================================================================
//static // BUGFIX
void DrawStatusArea (HDC hDC, int drawflags)
{
//	FrameReleaseDC();
//	HDC  g_hFrameDC     = (passg_hFrameDC ? passg_hFrameDC : GetDC(g_hFrameWindow));
	...
	if (g_bIsFullScreen)
	{
		...
		SetTextAlign(g_hFrameDC,TA_LEFT | TA_TOP);

		SetTextColor(g_hFrameDC, g_aDiskFullScreenColorsLED[ iDrive1Status ] );
		TextOut(g_hFrameDC,x+ 3,y+2,TEXT("1"),1);

		SetTextColor(g_hFrameDC, g_aDiskFullScreenColorsLED[ iDrive2Status ] );
		TextOut(g_hFrameDC,x+13,y+2,TEXT("2"),1);
		...
		//		DrawBitmapRect(g_hFrameDC,x+12,y+DriveLedY,&rect,g_hDiskFullScreenLED[ iDrive1Status ]);
		//		DrawBitmapRect(g_hFrameDC,x+30,y+DriveLedY,&rect,g_hDiskFullScreenLED[ iDrive2Status ]);
		//		SetTextColor(g_hFrameDC, g_aDiskFullScreenColors[ iDrive1Status ] );
		//		TextOut(g_hFrameDC,x+ 10,y+2,TEXT("*"),1);
		//		SetTextColor(g_hFrameDC, g_aDiskFullScreenColors[ iDrive2Status ] );
		//		TextOut(g_hFrameDC,x+ 20,y+2,TEXT("*"),1);

		if (!IS_APPLE2)
		{
			SetTextAlign(g_hFrameDC,TA_RIGHT | TA_TOP);
			SetTextColor(g_hFrameDC,(bCaps
				? RGB(128,128,128)
				: RGB(  0,  0,  0) ));
			TextOut(g_hFrameDC,x+BUTTONCX,y+2,TEXT("Caps"),4);
		}
		SetTextAlign(g_hFrameDC,TA_CENTER | TA_TOP);
		SetTextColor(g_hFrameDC,(g_nAppMode == MODE_PAUSED || g_nAppMode == MODE_STEPPING
			? RGB(255,255,255)
			: RGB(  0,  0,  0)));
		TextOut(g_hFrameDC,x+BUTTONCX/2,y+13,(g_nAppMode == MODE_PAUSED
			? TITLE_PAUSED
			: TITLE_STEPPING) ,8);
	}
	else
	{
		...
		{
			SelectObject(g_hFrameDC,GetStockObject(NULL_PEN));
			SelectObject(g_hFrameDC,btnfacebrush);
			Rectangle(g_hFrameDC,x,y,x+BUTTONCX+2,y+35);
			Draw3dRect(g_hFrameDC,x+1,y+3,x+BUTTONCX,y+31,0);
			SelectObject(g_hFrameDC,smallfont);
			SetTextAlign(g_hFrameDC,TA_CENTER | TA_TOP);
			SetTextColor(g_hFrameDC,RGB(0,0,0));
			SetBkMode(g_hFrameDC,TRANSPARENT);
			TextOut(g_hFrameDC,x+ 7,y+7,TEXT("1"),1);
			TextOut(g_hFrameDC,x+25,y+7,TEXT("2"),1);
		}
		if (drawflags & DRAW_LEDS)
		{
			RECT rect = {0,0,8,8};
			DrawBitmapRect(g_hFrameDC,x+12,y+8,&rect,g_hDiskWindowedLED[iDrive1Status]);
			DrawBitmapRect(g_hFrameDC,x+30,y+8,&rect,g_hDiskWindowedLED[iDrive2Status]);
			...
			switch (g_Apple2Type)
			{
			case A2TYPE_APPLE2:			DrawBitmapRect(g_hFrameDC,x+7,y+9,&rect,capsbitmap[bCaps != 0]); break; 
			case A2TYPE_APPLE2PLUS:		DrawBitmapRect(g_hFrameDC,x+7,y+19,&rect,capsbitmap[bCaps != 0]); break; 
			case A2TYPE_APPLE2E:		DrawBitmapRect(g_hFrameDC,x+7,y+19,&rect,capsbitmap[bCaps != 0]); break; 
			case A2TYPE_APPLE2EEHANCED:	DrawBitmapRect(g_hFrameDC,x+7,y+19,&rect,capsbitmap[bCaps != 0]); break; 
			case A2TYPE_PRAVETS82:		DrawBitmapRect(g_hFrameDC,x+15,y+19,&rect,latbitmap[bCaps != 0]); break; 
			case A2TYPE_PRAVETS8M:		DrawBitmapRect(g_hFrameDC,x+15,y+19,&rect,latbitmap[bCaps != 0]); break; 
			case A2TYPE_PRAVETS8A:		DrawBitmapRect(g_hFrameDC,x+2,y+19,&rect,latbitmap[bCaps != 0]); break; 
			}
			if (g_Apple2Type == A2TYPE_PRAVETS8A) //Toggles Pravets 8A/C Caps lock LED
			{
				RECT rect = {0,0,22,8};
				DrawBitmapRect(g_hFrameDC,x+23,y+19,&rect,capsbitmapP8[P8CAPS_ON != 0]);
			}

		
/*
			if (g_Apple2Type == A2TYPE_PRAVETS8A)
					DrawBitmapRect(g_hFrameDC,x+7,y+19,&rect,cyrbitmap[bCaps != 0]);
				else
					DrawBitmapRect(g_hFrameDC,x+7,y+19,&rect,capsbitmap[bCaps != 0]);
*/
			}
		}
		...
		if (drawflags & DRAW_TITLE)
		{
			...
			if( true ) // TODO: g_bDisplayVideoModeInTitle
			{
				_tcscat( title, " - " );
		...
		{
			DrawButton(g_hFrameDC, BTN_DRIVE1);
			DrawButton(g_hFrameDC, BTN_DRIVE2);
		}
	}
...
// @F9 -Can't use Alt-F9 as Alt is Open-Apple = Joystick Button #1
			...
			{
				// CRASH: Pressing F9 before emulator has begun...
				if( g_pfDebuggerRenderUpdate )
				{
					//VideoRedrawScreen(); // vvv
					g_bVideoForceFullRedraw = true;
...
*/
    ...
    case WM_USER_BENCHMARK: {
      ...
      DrawStatusArea((HDC)0,DRAW_TITLE);
      HCURSOR olg_hFrameDCursor = SetCursor(LoadCursor(0,IDC_WAIT));
      VideoBenchmark();
      ResetMachineState();
      SetCursor(olg_hFrameDCursor);
      break;
    }
...
/*
	...
	DrawFrameWindow
		...
		VideoRedrawScreen		
*/
	// HACK: DD Full-Screen Palette Bugfix: F6,F7,F7
...
HDC FrameGetDC ()
{
	...
	//			}
			...
			g_pFrameBuffer = (LPBYTE)surfacedesc.lpSurface + ((VIEWPORTCY-1)*surfacedesc.lPitch);
			g_nFramePitch  = -surfacedesc.lPitch;

			hResult = g_pDDPrimarySurface->GetDC( &g_hFrameDC );
			if( FAILED( hResult ) )
			{
				g_pFrameBuffer = NULL;
				g_nFramePitch = 0;
...
//			SetViewportOrgEx(g_hFrameDC,viewportx,viewporty,NULL);
			...
			g_pFrameBuffer = g_pFramebufferbits;
			g_nFramePitch  = FRAMEBUFFER_W;
		}

		Video_CreateFrameOffsetTable(g_pFrameBuffer,g_nFramePitch); // ptr to start of each scanline
	}
...
void FrameReleaseDC () 
{
	...
	if (g_bIsFullScreen)
	{
		if( g_bAppActive && !g_bPaintingWindow)
		{
			...
			hResult = g_pDDPrimarySurface->ReleaseDC( g_hFrameDC );
			if( FAILED( hResult ) )
			{
				return;
			}
...
void FrameReleaseVideoDC ()
{
	...
	if (g_bIsFullScreen )
	{
//		if( FAILED( hResult = m_pdds->ReleaseDC( hDC ) ) )
		{
			return;
		}
	}
...
void FrameRegisterClass () {
  WNDCLASSEX wng_hFrameDClass;
  ZeroMemory(&wng_hFrameDClass,sizeof(WNDCLASSEX));
  wng_hFrameDClass.cbSize        = sizeof(WNDCLASSEX);
  wng_hFrameDClass.style         = CS_OWNDC | CS_BYTEALIGNCLIENT;
  wng_hFrameDClass.lpfnWndProc   = FrameWndProc;
  wng_hFrameDClass.hInstance     = g_hInstance;
  wng_hFrameDClass.hIcon         = LoadIcon(g_hInstance,TEXT("APPLEWIN_ICON"));
  wng_hFrameDClass.hCursor       = LoadCursor(0,IDC_ARROW);
  wng_hFrameDClass.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
#if ENABLE_MENU
  wng_hFrameDClass.lpszMenuName	 = (LPCSTR)IDR_MENU1;
#endif
  wng_hFrameDClass.lpszClassName = TEXT("APPLE2FRAME");
  wng_hFrameDClass.hIconSm       = (HICON)LoadImage(g_hInstance,TEXT("APPLEWIN_ICON"),
                                            IMAGE_ICON,16,16,LR_DEFAULTCOLOR);
  RegisterClassEx(&wng_hFrameDClass);
}
