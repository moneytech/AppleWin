...
#define  MSW_80STORE    (memmode & MF_80STORE)
#define  MSW_ALTZP      (memmode & MF_ALTZP)
#define  MSW_AUXREAD    (memmode & MF_AUXREAD)
#define  MSW_AUXWRITE   (memmode & MF_AUXWRITE)
#define  MSW_BANK2      (memmode & MF_BANK2)
#define  MSW_HIGHRAM    (memmode & MF_HIGHRAM)
#define  MSW_HIRES      (memmode & MF_HIRES)
#define  MSW_PAGE2      (memmode & MF_PAGE2)
#define  MSW_SLOTC3ROM  (memmode & MF_SLOTC3ROM)
#define  MSW_SLOTCXROM  (memmode & MF_SLOTCXROM)
#define  MSW_WRITERAM   (memmode & MF_WRITERAM)
...
BYTE __stdcall IORead_Cxxx(WORD programcounter, WORD address, BYTE write, BYTE value, ULONG nCyclesLeft)
{
	if (address == 0xCFFF)
	{
		...
		if (MSW_SLOTCXROM)
		{
			// NB. MSW_SLOTCXROM==0 ensures that internal rom stays switched in
	...
	if (IS_APPLE2 || MSW_SLOTCXROM)
	{
		if ((address >= APPLE_SLOT_BEGIN) && (address <= APPLE_SLOT_END))
		{
			...
			if ((uSlot != 3) && ExpansionRom[uSlot])
				IO_SELECT |= 1<<uSlot;
			else if ((MSW_SLOTC3ROM) && ExpansionRom[uSlot])
				IO_SELECT |= 1<<uSlot;		// Slot3 & Peripheral ROM
			else if (!MSW_SLOTC3ROM)
	...
	if (!IS_APPLE2 && !MSW_SLOTCXROM)
	{
		// !MSW_SLOTC3ROM = Internal ROM: $C300-C3FF
		// !MSW_SLOTCXROM = Internal ROM: $C100-CFFF

		if ((address >= APPLE_SLOT_BEGIN) && (address <= APPLE_SLOT_END))	// Don't care about state of MSW_SLOTC3ROM
			IO_SELECT_InternalROM = 1;
		else if ((address >= FIRMWARE_EXPANSION_BEGIN) && (address <= FIRMWARE_EXPANSION_END))
			IO_STROBE = 1;

		if (!MSW_SLOTCXROM && IO_SELECT_InternalROM && IO_STROBE && (g_eExpansionRomType != eExpRomInternal))
		{
			// Enable Internal ROM
			memcpy(mem+FIRMWARE_EXPANSION_BEGIN, pCxRomInternal+0x800, FIRMWARE_EXPANSION_SIZE);
			g_eExpansionRomType = eExpRomInternal;
...
void UpdatePaging (BOOL initialize, BOOL updatewriteonly)
{
	...
	if (!updatewriteonly)
	{
		for (loop = 0x00; loop < 0x02; loop++)
			memshadow[loop] = MSW_ALTZP ? memaux+(loop << 8) : memmain+(loop << 8);
	}

	for (loop = 0x02; loop < 0xC0; loop++)
	{
		memshadow[loop] = MSW_AUXREAD ? memaux+(loop << 8)
			: memmain+(loop << 8);

		memwrite[loop]  = ((MSW_AUXREAD != 0) == (MSW_AUXWRITE != 0))
			? mem+(loop << 8)
			: MSW_AUXWRITE	? memaux+(loop << 8)
							: memmain+(loop << 8);
	}
	...
	if (!updatewriteonly)
	{
		for (loop = 0xC0; loop < 0xC8; loop++)
		{
			...
			if (loop == 0xC3)
				memshadow[loop] = (MSW_SLOTC3ROM && MSW_SLOTCXROM)	? pCxRomPeripheral+uSlotOffset	// C300..C3FF - Slot 3 ROM (all 0x00's)
																	: pCxRomInternal+uSlotOffset;	// C300..C3FF - Internal ROM
			else
				memshadow[loop] = MSW_SLOTCXROM	? pCxRomPeripheral+uSlotOffset						// C000..C7FF - SSC/Disk][/etc
	...
	{
		int bankoffset = (MSW_BANK2 ? 0 : 0x1000);
		memshadow[loop] = MSW_HIGHRAM ? MSW_ALTZP	? memaux+(loop << 8)-bankoffset
												: memmain+(loop << 8)-bankoffset
									: memrom+((loop-0xD0) * 0x100);

		memwrite[loop]  = MSW_WRITERAM	? MSW_HIGHRAM	? mem+(loop << 8)
														: MSW_ALTZP	? memaux+(loop << 8)-bankoffset
																	: memmain+(loop << 8)-bankoffset
										: NULL;
	}
	...
	{
		memshadow[loop] = MSW_HIGHRAM	? MSW_ALTZP	? memaux+(loop << 8)
													: memmain+(loop << 8)
										: memrom+((loop-0xD0) * 0x100);

		memwrite[loop]  = MSW_WRITERAM	? MSW_HIGHRAM	? mem+(loop << 8)
														: MSW_ALTZP	? memaux+(loop << 8)
																	: memmain+(loop << 8)
										: NULL;
	}
	...
	if (MSW_80STORE)
	{
		...
		{
			memshadow[loop] = MSW_PAGE2	? memaux+(loop << 8)
										: memmain+(loop << 8);
			memwrite[loop]  = mem+(loop << 8);
		}
...
BYTE __stdcall MemCheckPaging (WORD, WORD address, BYTE, BYTE, ULONG)
{
	...
	switch (address)
	{
	case 0x11: result = MSW_BANK2;       break;
	case 0x12: result = MSW_HIGHRAM;     break;
	case 0x13: result = MSW_AUXREAD;     break;
	case 0x14: result = MSW_AUXWRITE;    break;
	case 0x15: result = !MSW_SLOTCXROM;  break;
	case 0x16: result = MSW_ALTZP;       break;
	case 0x17: result = MSW_SLOTC3ROM;   break;
	case 0x18: result = MSW_80STORE;     break;
	case 0x1C: result = MSW_PAGE2;       break;
	case 0x1D: result = MSW_HIRES;       break;
	}
...
{
	return MSW_SLOTCXROM ? true : false;
}
...
#ifdef RAMWORKS
	if ( ((MSW_PAGE2 && MSW_80STORE) || VideoGetSW80COL()) &&
		( ( ((offset & 0xFF00)>=0x0400) &&
		((offset & 0xFF00)<=0700) ) ||
		( MSW_HIRES && ((offset & 0xFF00)>=0x2000) &&
