...
#define ANC	 regs.a &= READ;					    \
                 SETNZ(regs.a)						    \
                 flagc = flagn;
#define ARR	 regs.a &= READ; /*FIXME: obey D flag */         	    \
                 flagv	= (((regs.a & 0x80) == (temp & 0x80)) &&	    \
                             ((regs.a & 0x80) != (val & 0x80)));	    \
                 temp =	  regs.a;					    \
...
#define ASO	 bWrtMem = 1;						    \
                 ...
                 regs.a |= val;						    \
                 SETNZ(regs.a)
#define AXA	 bWrtMem = 1;/*FIXME: $93 case is unclear*/                 \
                 val = regs.a&regs.x&((*(LPWORD)(mem+regs.pc-1))+1 & 0xFF); \
                 WRITE(val)
...
#define SAY	 bWrtMem = 1;						    \
                 val = regs.y & ((*(LPWORD)(mem+regs.pc-1))+1 & 0xFF);	    \
                 WRITE(val)
...
#define TAS	 bWrtMem = 1;						    \
                 val = regs.a & regs.x;					    \
                 regs.sp = 0x100 | val;					    \
                 val &= ((*(LPWORD)(mem+regs.pc-1))+1 & 0xFF);		    \
                 WRITE(val)
...
#define XAS	 bWrtMem = 1;						    \
                 val = regs.x & ((*(LPWORD)(mem+regs.pc-1))+1 & 0xFF);	    \
                 WRITE(val)
